...
# -----------------------------------------
# HAND-TUNED STARTING PRESTIGE VALUES
# These replace the calc_prestige() formula output for known programs.
# Based on real program identity: tournament history, current relevance,
# recruiting pull, conference standing, and program reputation.
#
# Scale reference:
#   95-100: Blue blood (Kentucky, Kansas, Duke, UNC) -- 4 programs max
#   79-94:  Elite (top 15-18 nationally relevant programs)
#   59-78:  Strong (consistent tournament teams, recruiting destinations)
#   39-58:  Average (bubble teams, mid-major powers, rebuilding power programs)
#   21-38:  Below average (long-shot tournaments, rebuilding programs)
#   8-20:   Poor / floor conf (rarely tournament-relevant)
# -----------------------------------------
...
PRESTIGE_OVERRIDES = {
    ...
    # ── BELOW AVERAGE: MID-MAJOR PROGRAMS ────────────────────────────────
    "Akron":          46,
    "Buffalo":        44,
    "Ohio":           42,
    "Toledo":         41,
    ...
    "Incarnate Word": 18,
    "Texas A&M-Corpus Christi": 20,

    # ── MISSING OVERRIDES (were falling back to formula) ─────────────────
    # Power conference programs with real identities
    "Clemson":            52,
    "Virginia Tech":      55,
    "Northwestern":       54,
    "USC":                62,
    ...
    "Wagner":             13,
    "Stonehill":          10,
    "St. Francis Brooklyn": 11,
    "Abilene Christian":  16,
    "Tarleton State":     12,
    "UT Arlington":       15,
    "Utah Valley":        15,
    "Dixie State":        12,
    "California Baptist": 14,
    "Seattle U":          13,
    "UMKC":               12,
}
...
def build_all_d1_programs():
    ...
    for data in ALL_D1_PROGRAMS:
        # Use hand-tuned override if available, otherwise fall back to formula
        if data["name"] in PRESTIGE_OVERRIDES:
            prestige = PRESTIGE_OVERRIDES[data["name"]]
            # Clamp to conference floor (never start below it)
            conf_floor = get_conference_floor(data["conference"])
            prestige = max(conf_floor, prestige)
        else:
